...
public class AlerteHome {
	...
	public Alerte findById(java.lang.String id) {
		log.debug("getting Alerte instance with id: " + id);
		try {
			Alerte instance = (Alerte) sessionFactory.getCurrentSession().get("model.Alerte", id);
			if (instance == null) {
	...
	public List<Alerte> findByExample(Alerte instance) {
		...
		try {
			List<Alerte> results = (List<Alerte>) sessionFactory.getCurrentSession().createCriteria("model.Alerte")
